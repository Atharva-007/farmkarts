...
class _InventoryPageState extends State<InventoryPage>
  ...
  /// Load inventory asynchronously with proper error handling
  Future<void> _loadInventory() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ToastHelper.showError(context, 'Failed to load inventory: ${e.toString()}');
      }
    }
  }
  ...
  void _calculateStatistics() {
    ...
    _totalValue = _allProducts.fold<double>(
      0,
  ...
  void _navigateToAddProduct() async {
    try {
      ...
      if (mounted) {
        ToastHelper.showError(context, 'Failed to navigate: $e');
      }
    }
  }
  ...
  void _editProduct(Product product) async {
    try {
      ...
      if (mounted) {
        ToastHelper.showError(context, 'Failed to edit product: $e');
      }
    }
  }
    ...
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Text('Add stock to all active products'),
            const SizedBox(height: 16),
            TextField(
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (quantity != null && quantity > 0) {
                Navigator.pop(context);
                await _performBulkStockUpdate(quantity);
              }
            },
  ...
  Future<void> _performBulkStockUpdate(int quantity) async {
    try {
      final batch = _firestore.batch();
      
      for (final product in _allProducts.where((p) => p.isAvailable)) {
      ...
      if (mounted) {
        ToastHelper.showSuccess(context, 'Stock updated successfully');
        _loadInventory();
      }
    } catch (e) {
      if (mounted) {
        ToastHelper.showError(context, 'Failed to update stock: $e');
      }
    }
  }

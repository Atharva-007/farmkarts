...
class TrendingVideoService {
  ...
  static final TrendingVideoService _instance =
  ...
  Stream<List<TrendingVideo>> getTrendingVideos(
    ...
    Stream<List<TrendingVideo>> trendingStream = trendingQuery
      ...
      return snapshot.docs
          .map((doc) =>
              TrendingVideo.fromMap(doc.id, doc.data() as Map<String, dynamic>))
          .toList();
    });

    // Stream 2: Videos from Community Feed
    Query feedQuery = _firestore
    ...
    return CombineLatestStream.combine2<List<TrendingVideo>,
        List<TrendingVideo>, List<TrendingVideo>>(
      trendingStream,
      feedVideosStream,
      (trending, feed) {
        ...
        if (search != null && search.isNotEmpty) {
          return uniqueVideos
              .where(
                  (v) => v.title.toLowerCase().contains(search.toLowerCase()))
              .toList();
        }
  ...
  Future<List<TrendingVideo>> getTrendingFromBackend() async {
    try {
      final response = await http
          .get(Uri.parse('$_baseUrl/trending'))
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          return videos
              .map((v) =>
                  TrendingVideo.fromMap(v['youtubeId'] ?? v['id'] ?? '', v))
              .toList();
        }
      }
      return _getMockVideos();
  ...
  Future<void> updateEngine() async {
    try {
      ...
      if (lastUpdateDoc.exists) {
        final lastUpdate =
            (lastUpdateDoc.data()?['lastSync'] as Timestamp).toDate();
